...
void FarblingTabHelper::DidRedirectNavigation(
    content::NavigationHandle* navigation_handle) {
  UpdateUserAgent(navigation_handle);
}
...
void FarblingTabHelper::UpdateUserAgent(
  ...
  // |ua| might still be an empty string here. Passing an empty string to
  // SetUserAgentOverride means "do not override," not "send an empty string."
